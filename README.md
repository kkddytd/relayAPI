...
Compose 默认将网页绑定到 `0.0.0.0:6722`，网页本身允许直接通过公网地址打开；检测 API 和旧版附件 API 仍需要 Bearer Key（或受控反向代理/已验证网页会话）。公网部署时请将 `ALLOW_LAN_WEB_WITHOUT_TURNSTILE=false`，并使用 HTTPS 反向代理。所有历史、附件和安装统计数据都保存在 `data/`，升级时不要删除该目录。
