...
```bash
git clone https://github.com/<你的 GitHub 账号>/relayAPI.git
cd relayAPI
bash scripts/deploy.sh
```

也可以使用 npm 别名：`npm run deploy`。
...
`npm run start` 会同时启动检测网页/API 和本机安装统计服务；只启动检测服务时使用 `npm run start:web`。开发模式使用 `npm run dev`。
