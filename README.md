...
也可以使用 npm 命令：`npm run deploy:docker`（兼容别名：`npm run deploy`）。
...
Compose 默认允许受控局域网直接访问网页，公网部署前请在 `.env` 中配置 `DETECTOR_API_KEYS`，并将 `ALLOW_LAN_WEB_WITHOUT_TURNSTILE=false`。所有历史、附件和安装统计数据都保存在 `data/`，升级时不要删除该目录。
...
本地 Node.js 需要让局域网访问时，在 `.env` 设置 `HOST=0.0.0.0`；Docker Compose 已默认监听容器内的 `0.0.0.0`。更换对外端口时，Docker 使用 `RELAYAPI_PORT=8080 bash scripts/deploy.sh`，本地 Node.js 使用 `PORT=8080 HOST=0.0.0.0 bash scripts/deploy-local.sh`。安装统计服务仍使用内部端口 `6723`。
