# relayAPI | AI Model Quality Testing, LLM Evaluation & API Compatibility

> relayAPI 是一个面向 Claude、GPT、Gemini、GLM 和自定义模型的开源 AI 模型质量检测、LLM 评测与 API 兼容性测试平台。

Open-source and self-hosted tooling for AI model quality testing, LLM evaluation, OpenAI-compatible and Anthropic-compatible API testing, model gateway monitoring, prompt-cache checks, attachment recognition, and regression testing. Run it locally with a REST API, SQLite, Docker Compose, or Node.js.
...
关键词：AI 模型质量检测、LLM 评测、Claude 检测、GPT 质量测试、Gemini API 兼容性、自定义模型 ID、模型中转站检测、API 协议探针、提示缓存检测、附件识别、API 质量监测、本地化部署、Docker Compose、SQLite、开源 AI 工具。

English keywords: AI model quality testing, LLM evaluation, LLM benchmark, Claude compatibility, Anthropic API testing, GPT API testing, OpenAI API compatibility, Gemini API testing, Google Gemini, model gateway monitoring, custom model ID, protocol probe, prompt cache validation, attachment recognition, REST API, local deployment, Docker Compose, SQLite.
...
项目不依赖第三方评测页面，检测请求、评分逻辑和数据存储均可自行检查与部署。它适合模型接入调试、API 网关验收、模型版本回归、AI 中转站质量监测、私有模型验收和团队内部的 LLM 评测。
...
- **模型中转站验收**：比较同一模型在不同网关、反代或 API 聚合渠道中的响应质量与协议兼容性。
- **自定义模型回归**：保留上游要求的原始 `model` 字符串，同时指定本地评测档案，适合私有模型和厂商自定义 ID。
- **多协议兼容测试**：验证 Anthropic Messages、OpenAI Chat、OpenAI Responses、OpenAI Images 和 Google Generative 路线。
- **附件能力检查**：上传图片、PDF、JSON、文本或源代码，只判断附件是否进入请求并形成识别证据，不把附件结果混入主评分。
- **本地审计与复测**：将历史请求、API 调用记录和加密凭据保存在本机 SQLite，支持网页一键复测和结果对比。

## 常见问题

### relayAPI 检测的是模型质量还是模型身份？

主分优先反映响应质量和公开兼容性证据；渠道、行为和来源字段独立展示。满分不等于模型身份的密码学证明。

### 自定义模型名可以检测吗？

可以。`model` 填写上游实际需要的任意模型 ID，`profile_model` 选择最接近的本地评测档案。两者会在报告中分别保存，适合中转站和私有模型。

### 上传附件会改变模型主评分吗？

不会。附件检查独立判断模型是否收到并识别附件，不判断内容语义是否正确，也不参与主评分。附件原文件单独保存，历史复测仍可使用。

### API Key 会上传到第三方吗？

检测服务只在执行检测时将上游 Key 发往调用方指定的目标接口；本机历史中的完整请求使用 AES-256-GCM 加密，网页和历史响应不会展示上游 Key。

### 可以不使用 Docker 吗？

可以。Node.js 部署脚本支持 Node.js 20.19+ 或 22.12+，Docker Compose 和本地后台部署均提供一键命令，详细步骤见[快速开始](#快速开始)。
